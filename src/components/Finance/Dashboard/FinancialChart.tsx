...
export const FinancialChart: React.FC<FinancialChartProps> = ({
  ...
  const chartData = useMemo(() => {
    ...
    return intervals.map(intervalDate => {
      const periodLabel = format(intervalDate, formatPattern, {
        locale: ptBR
      });
      ...
      const entradas = periodEntries.filter(entry => entry.type === 'receivable').reduce((sum, entry) => sum + Number(entry.amount), 0);
      const saidas = periodEntries.filter(entry => entry.type === 'payable').reduce((sum, entry) => sum + Number(entry.amount), 0);
      return {
        period: periodLabel,
        entradas,
        saidas
        ...
        <div className="h-80 w-full">
          <ResponsiveContainer width="100%" height="100%">
            ...
            bottom: 5
              ...
              <YAxis className="text-muted-foreground" fontSize={12} tickFormatter={value => formatCurrency(value)} />
