-- Fix duplicate entry_number by seeding company sequence from existing max and ensuring trigger exists
-- 1) Replace generate_sequence_number to properly initialize per-table sequences
...
-- 2) Ensure BEFORE INSERT trigger exists on financial_entries to set entry_number
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_financial_entries_set_number'
  ) THEN
    CREATE TRIGGER trg_financial_entries_set_number
    BEFORE INSERT ON public.financial_entries
    FOR EACH ROW EXECUTE FUNCTION public.set_entry_number();
  END IF;
END $$;

-- 3) Keep sales flow trigger safeguarded (create if missing)
DO $$
BEGIN
  IF NOT EXISTS (
