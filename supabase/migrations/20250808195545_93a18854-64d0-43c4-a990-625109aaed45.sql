...
-- 2) Backfill company_id from profiles based on user_id, skipping approved rows to avoid blocking trigger
UPDATE public.packaging p
SET company_id = pr.company_id
FROM public.profiles pr
WHERE p.company_id IS NULL AND pr.id = p.user_id AND COALESCE(p.status::text, '') <> 'approved';

-- 3) Ensure company_id is auto-set on new rows
...
-- 4) Replace global unique constraint with per-company uniqueness
...
-- 5) Optional index to keep fast lookups by packaging_number within company
