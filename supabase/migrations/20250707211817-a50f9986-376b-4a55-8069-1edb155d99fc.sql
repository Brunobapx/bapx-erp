-- Verificar se o tipo master existe, se não criar
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'user_type' AND 'master' = ANY(enum_range(NULL::user_type)::text[])) THEN
        ALTER TYPE user_type ADD VALUE 'master';
    END IF;
END$$;
